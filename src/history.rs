...
/// Counter Move History - tracks good responses to specific moves
...
impl CounterMoveHistory {
    ...
    pub fn get(&self, prev_piece: Piece, prev_to: Square, curr_piece: Piece, curr_to: Square) -> i32 {
        self.entries[prev_piece][prev_to][curr_piece][curr_to] as i32
    }

    pub fn update(&mut self, prev_piece: Piece, prev_to: Square, curr_piece: Piece, curr_to: Square, bonus: i32) {
        let entry = &mut self.entries[prev_piece][prev_to][curr_piece][curr_to];
        apply_bonus::<{ Self::MAX_HISTORY }>(entry, bonus);
    }
}
