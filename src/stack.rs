...
impl Stack {
    ...
    /// Reset stack entries up to max_ply without reallocating
    /// This is faster than creating a new Stack every aspiration window
    pub fn reset(&mut self) {
        let ptr = &raw mut self.sentinel;
        for entry in &mut self.data {
            // Only reset fields that matter between searches
            entry.mv = Move::NULL;
            entry.piece = Piece::None;
            entry.eval = Score::NONE;
            entry.excluded = Move::NULL;
            entry.tt_move = Move::NULL;
            entry.tt_pv = false;
