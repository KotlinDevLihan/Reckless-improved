...
pub fn start(td: &mut ThreadData, report: Report, thread_count: usize) {
    ...
    for depth in 1..MAX_PLY as i32 {
        ...
        let multiplier = || {
            ...
            let complexity = td.board.occupancies().popcount() as f32 / 32.0;
            let phase_factor = if complexity > 0.7 { 1.15 } else if complexity > 0.4 { 1.0 } else { 0.9 };

            // Trend-based adjustment - use more time when position is unstable
...
fn search<NODE: NodeType>(
    ...
    // Static Null Move Pruning (also called eval-based pruning)
    if !NODE::PV
        && !in_check
        && !excluded
        && depth < 4
        && eval < alpha - 128 * depth
    {
        return qsearch::<NonPV>(td, alpha, beta, ply);
    }

    // Razoring - enhanced with improving flag
    if !NODE::PV
        && !in_check
        && estimated_score < alpha - 295 - 261 * depth * depth
        && alpha < 2048
        && !tt_move.is_quiet()
        && !improving  // Don't razor when position is improving
    {
        return qsearch::<NonPV>(td, alpha, beta, ply);
    }
    ...
    // Internal Iterative Deepening (IID) - do shallow search to get a good tt_move
    if !NODE::ROOT && tt_move.is_null() && depth >= 5 {
        let iid_depth = (depth - 2).max(1);
        search::<NonPV>(td, alpha, beta, iid_depth, cut_node, ply);
        // Re-read TT after IID to get the best move
        let new_entry = td.shared.tt.read(hash, td.board.halfmove_clock(), ply);
        if let Some(entry) = new_entry {
            if entry.depth >= iid_depth - 1 {
                tt_move = entry.mv;
            }
        }
    }
    ...
    // Check Extensions - extend when in check to find more tactical shots
    ...
    while let Some(mv) = move_picker.next::<NODE>(td, skip_quiets, ply) {
        ...
        if !NODE::ROOT && !is_loss(best_score) {
            ...
            // History Leaf Pruning (HLP) - prune quiet moves with very bad history
            if !in_check
                && is_quiet
                && depth <= 3
                && history < -4096 * depth
        ...
        if depth >= 2 && move_count >= 2 {
            ...
            // Threat-aware LMR: reduce less when move creates threats
    ...
    if best_move.is_present() {
        ...
        if best_move.is_noisy() {
            ...
            if prev_mv.is_present() {
                ...
                let curr_piece = td.board.moved_piece(best_move);
                let curr_to = best_move.to();
                td.counter_move_history.update(prev_piece, prev_to, curr_piece, curr_to, quiet_bonus);
            }
...
fn qsearch<NODE: NodeType>(td: &mut ThreadData, mut alpha: i32, beta: i32, ply: isize) -> i32 {
    ...
    // Deferred evaluation: check if we can skip expensive NNUE call
    const QSEARCH_EVAL_MARGIN: i32 = 200;
    if !in_check && is_valid(tt_score) && !is_decisive(tt_score) {
        // If TT score suggests we'll fail high, use it directly
        if tt_score >= beta + QSEARCH_EVAL_MARGIN {
            return tt_score;
        }
    }
            ...
            if score > alpha {
                ...
                if score >= beta {
                    ...
                    if best_move.is_noisy() {
                        ...
                        // Update counter move history in qsearch
                        let prev_mv = td.stack[ply - 1].mv;
                        if prev_mv.is_present() {
                            let prev_piece = td.stack[ply - 1].piece;
                            let prev_to = prev_mv.to();
                            let curr_piece = td.board.moved_piece(best_move);
                            let curr_to = best_move.to();
                            td.counter_move_history.update(prev_piece, prev_to, curr_piece, curr_to, bonus);
                        }
                    }
