...
impl Board {
    ...
    pub fn checking_squares(&self, pt: PieceType) -> Bitboard {
        // Guard against PieceType::None which has index 6 but array only has 6 elements
        if pt as usize >= PieceType::NUM {
            return Bitboard(0);
        }
    ...
    pub fn piece_threats(&self, pt: PieceType) -> Bitboard {
        // Guard against PieceType::None which has index 6 but array only has 6 elements
        if pt as usize >= PieceType::NUM {
            return Bitboard(0);
        }
