use crate::{
    ...
    search::NodeType,
    thread::ThreadData,
    types::{ArrayVec, Bitboard, MAX_MOVES, Move, MoveEntry, MoveList, Piece, PieceType, Square},
};
...
impl MovePicker {
    ...
    fn get_best_entry(&mut self) -> MoveEntry {
        ...
        // Remove uses swap_remove internally (O(1) vs O(n))
    ...
    fn score_quiet(&mut self, td: &ThreadData, ply: isize) {
        ...
        // Get previous move for counter move history
        let prev_mv = td.stack[ply - 1].mv;
        let prev_piece = if prev_mv.is_present() { td.stack[ply - 1].piece } else { Piece::None };
        let prev_to = if prev_mv.is_present() { prev_mv.to() } else { Square::A1 };
        ...
        for entry in self.list.iter_mut() {
            ...
            // Skip if no piece (shouldn't happen for valid moves)
            if pt == PieceType::None {
                continue;
            }

            // Counter move history bonus
            let cmh_bonus = if prev_mv.is_present() {
                td.counter_move_history.get(prev_piece, prev_to, piece, mv.to())
            } else { 0 };
            ...
            entry.score = 2048 * td.quiet_history.get(threats, side, mv) / 1024
                ...
                + 1024 * cmh_bonus / 1024  // Counter move history
